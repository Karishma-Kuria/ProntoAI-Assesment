...
class gitIntegration:
    def __init__(self):
        # add the path of the local git repo to Repo object of gitpython
        try:
            if len(sys.argv) == 2 and sys.argv[1] != "":
                isExist = os.path.exists(sys.argv[1])
                if isExist: 
                    self.repo = git.Repo(sys.argv[1])
                else:
    ...
    '''
        Code to get the active branch of the current repo.
    ...
    ''' Check if any files in the current branch is changed. 
    ...
    '''
        Get all the commits for the active branch. This will give us the list of all the commits in a chronological order 
        of the current branch with the latest commit first.
    '''
    def getRecentCommit(self, branch):
        # gets the list of the lastest commit for given branch
...
if __name__ == "__main__":
    ...
    print("current commit:", str(recent_commit))
